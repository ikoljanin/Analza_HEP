...
void Lifetime::Loop()
{
	t_histo=new TH1F("Time","Time",28,0,10);
   ...
   cout<<"Analiticki odredena vrijednost vremena poluzivota je   "<<suma_t/nentries<<endl;
   t_canvas->cd(1);
   ...
   //pogreška za ovu metodu dobivena je u "2.sigma.pdf"
	...
	t_canvas->cd(3);
	gPad->SetLeftMargin(0.18);
	lnL=new TF1("lnL","2*log(x*[0])+2*[1]/x",1.18, 1.3);//parametar funkcije je  tau (vrime poluživota); [0] i [1] su nentries i suma vremena (papiri izvedeno)
	lnL->SetParameter(0,N);
	lnL->SetParameter(1,suma_t);
...
void Lifetime::tau_bin()
{
	...
	for(bin_number=11;bin_number<=200;bin_number++)//200 je odabran na temelju pokušaja; nakon 200 sve točke su relativno blizu 
	{
		...
		cout<<bin_number<<"\t"<<tau_fit->GetParameter(1)-suma_t/N<<endl;
		if(abs(tau_fit->GetParameter(1)-suma_t/N)<=diff)
		{
			diff=abs(tau_fit->GetParameter(1)-suma_t/N);
			cout<<diff<<endl;
			opt_bin=bin_number;
		}
